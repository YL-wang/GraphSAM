...
def loss_landscape(checkpoint, train_dataset, model_params, train_params):
    # build loader
    train_loader = DataLoader(dataset=train_dataset, batch_size=train_params['batch_size'], collate_fn=mol_collate_func,
                              shuffle=True, drop_last=True, num_workers=4, pin_memory=True)
    ...
    # compute loss data
        loss_data = loss_landscapes.linear_interpolation(model_initial, model_final, loss, STEPS, deepcopy_model=True)
        ...
        plt.ylabel('Loss')
        axes = plt.gca()
        plt.savefig('losslandscapes.pdf', bbox_inches="tight", dpi=1800)
...
if __name__ == '__main__':
    ...
    if args.split == 'scaffold':
        ...
        for idx in range(args.fold):
            ...
            _ = loss_landscape(checkpoint, test_dataset, model_params, train_params)
            # # test
