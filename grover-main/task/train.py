...
def train(epoch, model, data, loss_func, optimizer, scheduler,
    ...
    if args.sam == 1:
        for _, item in enumerate(mol_loader):
            ...
            for name, parms in model.named_parameters():
                if name=='mol_atom_from_atom_ffn.1.weight':  #mol_atom_from_atom_ffn.1.weight
                    gs = parms.grad[0].cpu()
                    gs_list.append(gs)
            ...
            # g_norm = torch.norm(g1)
            #
            # with open('/root/grover-main/gs1norm.txt', 'a', encoding='utf-8') as f:
            #    f.write("%.4f"%g_norm)
            #    f.write(',')
        ...
        for _, item in enumerate(mol_loader):
            ...
            if i==0:
                ...
                optimizer.step(i, closure, loss)
            else:
                optimizer.step(i, closure)
        ...
        for _, item in enumerate(mol_loader):
            ...
            #         g_norm = torch.norm(g1)
            #
            #         with open('/root/grover-main/g1.txt', 'a', encoding='utf-8') as f:
            #            f.write("%.4f"%g_norm)
            #            f.write(',')
            optimizer.step()
    ...
    # cos_sim2 = nn.CosineSimilarity(dim=0, eps=1e-6)
    ...
    #
    # for j in range(0, len(gs_list)-1):
    #     a = F.normalize(gh_list[j+1], p=2, dim=0)
    #     b = F.normalize(gs_list[j], p=2, dim=0)
    #     # with open('/root/grover-main/consine.txt', 'a', encoding='utf-8') as f:
    #     #    f.write("%.4f"%cosine(F.normalize(gh[i+1], p=2, dim=0), F.normalize(gs[i], p=2, dim=0)))
    #     #    f.write(',')
    #     with open('/root/grover-main/similar.txt', 'a', encoding='utf-8') as f:
    #        #f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))
    #
    #        f.write("%.4f" % cos_sim(F.normalize(gh_list[j+1], p=2, dim=0), F.normalize(gs_list[j], p=2, dim=0)))
    #        f.write(',')
    #
    #     with open('/root/grover-main/ghnorm.txt', 'a', encoding='utf-8') as f:
    #         # f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))
    #
    #         f.write("%.4f" % torch.norm(gh_list[j+1]-gh_list[j]))
    #         f.write(',')
    #     with open('/root/grover-main/gsnorm.txt', 'a', encoding='utf-8') as f:
    #         # f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))
    #
    #         f.write("%.4f" % torch.norm(gs_list[j+1]-gs_list[j]))
    #         f.write(',')
        #
        # with open('/root/grover-main/similar4.txt', 'a', encoding='utf-8') as f:
        #    f.write("%.4f"%cos_sim2(F.normalize(gh[i+1], p=2, dim=0), F.normalize(gs[i], p=2, dim=0).t()))
        #    f.write(',')
...
def run_training(args: Namespace, time_start, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for j in range(0, len(gs_list) - 1):
            ...
            #    f.write("%.4f"%cosine(F.normalize(gh[i+1], p=2, dim=0), F.normalize(gs[i], p=2, dim=0)))
            #    f.write(',')
            gh_2 = alpha*gh_list[j] + (1-alpha)*(gs_list[j]/torch.norm(gs_list[j] + 1e-8))
            g_list.append(gh_2)

            gh_2b =gh_2/(1-pow(alpha,j+1))
            g_bias.append(gh_2b)
            with open('/root/grover-main/similar1.txt', 'a', encoding='utf-8') as f:
               #f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))

               f.write("%.4f" % cos_sim(F.normalize(gh_list[0], p=2, dim=0), F.normalize(gh_list[j+1], p=2, dim=0)))
               f.write(',')

            with open('/root/grover-main/ghnorm1.txt', 'a', encoding='utf-8') as f:
                # f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))

                f.write("%.4f" % torch.norm(gh_list[0]-gh_list[j+1]))
                f.write(',')

            with open('/root/grover-main/similar2.txt', 'a', encoding='utf-8') as f:
                f.write("%.4f" % cos_sim(F.normalize(g_list[j+1], p=2, dim=0), F.normalize(gh_list[j+1], p=2, dim=0)))
                f.write(',')

            with open('/root/grover-main/ghnorm2.txt', 'a', encoding='utf-8') as f:
                # f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))

                f.write("%.4f" % torch.norm(g_list[j+1]-gh_list[j+1]))
                f.write(',')

            with open('/root/grover-main/similarb.txt', 'a', encoding='utf-8') as f:
                f.write("%.4f" % cos_sim(F.normalize(g_bias[j+1], p=2, dim=0), F.normalize(gh_list[j+1], p=2, dim=0)))
                f.write(',')

            with open('/root/grover-main/ghnormb.txt', 'a', encoding='utf-8') as f:
                # f.write("%.4f"%cos_sim(F.normalize(gh[j], p=2, dim=0), F.normalize(gh[j+1], p=2, dim=0)))

                f.write("%.4f" % torch.norm(g_bias[j+1]-gh_list[j+1]))
                f.write(',')
