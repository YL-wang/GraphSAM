...
def train(epoch, model, data, loss_func, optimizer, scheduler,
    ...
    for _, item in enumerate(mol_loader):
        ...
        if args.sam == 1:
            ...
            #     if name == 'mol_atom_from_atom_ffn.1.weight':
            #         gs=parms.grad[0].cpu()
            #         gs_norm = torch.norm(gs)
            #
            #         with open('/root/grover-main/gs2.txt', 'a', encoding='utf-8') as f:
            #             f.write("%.4f" % gs_norm)
            #             f.write(',')
            #
            #         thea = np.dot(g,gs)/(torch.norm(g)*torch.norm(gs))
            #         gv = gs-torch.norm(gs)*thea*g/torch.norm(g)
            #         gv_norm = torch.norm(gv)
            #
            #         with open('/root/grover-main/gv3.txt', 'a', encoding='utf-8') as f:
            #             f.write("%.4f" % gv_norm)
            #             f.write(',')
