...
class GraphSAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, arg, adaptive=True, **kwargs):
        assert arg.rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"
    ...
    def first_step(self, zero_grad=False, i=0):
        ...
        # if i == 0:
        #     self.gradh = self.param_groups
        #
        # for group in self.gradh:
        #
        #     scale = group["rho"] / (grad_norm+ 1e-12)
        #     import ipdb
        #     ipdb.set_trace()
    ...
    @torch.no_grad()
    def step(self, i, closure=None, loss=None):
        assert closure is not None, "Sharpness Aware Minimization requires closure, but it was not provided"
        closure = torch.enable_grad()(closure)  # the closure should do a full forward-backward pass
        if i == 0:
            loss.backward()
...
class MA_SAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=0.05, adaptive=True, radius=0.05, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(MA_SAM, self).__init__(params, defaults)
        self.rho = rho
        self.radius = radius
        self.base_optimizer = base_optimizer(self.param_groups, **kwargs)
        self.param_groups = self.base_optimizer.param_groups
